...
int* CreateMatrix(const int& nCol, const int& nRow)
{
	int *p = new int[nCol * nRow + 2];
	*p = nRow;
	p++;
	*p = nCol;
	p++;
...
void GenerateMatrix(int* m, const int& min, const int& max)
{
	int nRow, nCol;
	m = m - 2;
	nRow = *m;
	...
	for (int i = 0; i < nRow; i++)
		...
		{
			m[i * nCol + j] = GenerateRandom(min, max);
		}
}
...
int oddElement(int* m)
{
	...
	for (int i = 0; i < nRow; i++)
		for (int j = 0; j < nCol; j++)
		{
			if (isPrime(m[i * nCol + j])==true && m[i * nCol + j] > max)
				max = m[i * nCol + j];
...
{
	srand(4007);
	int m = GenerateRandom(6, 10);
	int n = GenerateRandom(4, 8);
	int* matrix = CreateMatrix(m, n);
	GenerateMatrix(matrix, 4, 50);
	PrintMatrix(matrix);
	cout << endl << "max element= " << oddElement(matrix) << endl;
	FreeMatrix(matrix);
	system("pause");
	return 0;
}*/
